# OASIS Apps: Interactive Tools in Claude Desktop

OASIS Apps bring interactivity to data exploration workflows. While traditional MCP tools return text responses, OASIS Apps render interactive UIs directly within your AI client — enabling real-time exploration, visual feedback, and iterative refinement without switching applications.

## How OASIS Apps Work

OASIS Apps use the MCP Apps protocol to serve interactive UIs alongside tool responses. When you call an app tool:
...
- **OASIS initialized**: An active dataset (`oasis init vf-ghana --src ...`)
- **MCP connection**: OASIS configured in your client (`oasis config claude`)

Apps gracefully degrade in hosts without MCP Apps support — you'll get text-based results instead of the interactive UI.

## Building an OASIS App

Apps live in `src/oasis/apps/`. Each app has:
...
```bash
cd src/oasis/apps/desert_mapper/ui
npm install
...
Register the app in `src/oasis/apps/__init__.py`:
