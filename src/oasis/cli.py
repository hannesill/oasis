...
app = typer.Typer(
    name="oasis",
    help="OASIS CLI: Initialize local datasets and manage the MCP server.",
...
def main_callback(
    ...
    verbose: Annotated[
        bool,
        typer.Option(
            "--verbose", "-V", help="Enable DEBUG level logging for OASIS components."
        ),
    ] = False,
):
    """
    Main callback for the OASIS CLI. Sets logging level.
    """
    oasis_logger = logging.getLogger("oasis")
    if verbose:
        oasis_logger.setLevel(logging.DEBUG)
        for handler in oasis_logger.handlers:
            handler.setLevel(logging.DEBUG)
        logger.debug("Verbose mode enabled via CLI flag.")
    else:
        oasis_logger.setLevel(logging.INFO)
        for handler in oasis_logger.handlers:
...
def dataset_init_cmd(
    ...
    from oasis.config import _ensure_custom_datasets_loaded
    ...
    # Check if oasis_data exists in a parent directory
    from oasis.config import _find_project_root_from_cwd

    cwd = Path.cwd()
    found_root = _find_project_root_from_cwd()

    # If we found oasis_data in a parent directory, ask user what to do
    if found_root != cwd:
        existing_data_dir = found_root / "oasis_data"
        console.print()
        warning(f"Found existing oasis_data at: {existing_data_dir}")
        print_key_value("Current directory", cwd)
        console.print()
        console.print("  [bold]1.[/bold] Use existing location")
        console.print("  [bold]2.[/bold] Create new oasis_data in current directory")
        ...
        if choice == "2":
            ...
            os.environ["OASIS_DATA_DIR"] = str(cwd / "oasis_data")
            success(f"Will create new oasis_data in {cwd / 'oasis_data'}")
        else:
            success(f"Will use existing oasis_data at {existing_data_dir}")
...
def use_cmd(
    ...
    if not availability["parquet_present"]:
        ...
        console.print(
            "  [muted]For DuckDB (local), run:[/muted] [command]oasis init[/command]"
        )
    else:
...
def status_cmd(
    ...
    if not active:
        ...
        console.print()
        console.print(
            "[muted]Set one with:[/muted] [command]oasis use <dataset>[/command]"
        )
        console.print(
            "[muted]List all with:[/muted] [command]oasis status --all[/command]"
        )
        return
    ...
    if not ds_info:
        ...
        console.print(
            f"  [muted]Initialize with:[/muted] [command]oasis init {active}[/command]"
        )
        return
    ...
    if ds_info["db_present"] and ds_def and ds_def.primary_verification_table:
        try:
            ...
            if "No files found" in str(e) or "no such file" in str(e).lower():
                ...
                console.print(
                    f"  [muted]Try:[/muted] [command]oasis init {active} --force[/command]"
                )
...
def skills_cmd(
    ...
    """
    Install OASIS skills for AI coding tools.

    Skills teach AI assistants how to use the OASIS Python API effectively.
    Supports Claude Code, Cursor, Cline, Codex CLI, Gemini CLI, and GitHub Copilot.
    """
    from oasis.skills import (
        AI_TOOLS,
    ...
    if list_installed:
        ...
        if not installed:
            console.print("[muted]No OASIS skills installed.[/muted]")
            console.print()
            console.print("[muted]Install with:[/muted] [command]oasis skills[/command]")
            return

        console.print()
        console.print("[bold]Installed OASIS skills:[/bold]")
        console.print()
...
def _prompt_select_tools() -> list[str]:
    ...
    from oasis.skills import AI_TOOLS
...
def config_cmd(
    ...
    skills: Annotated[
        ...
        typer.Option(
            "--skills",
            help="Install OASIS skills after config.",
        ),
    ] = False,
):
    """
    Configure OASIS MCP server for various clients.

    Examples:

    * oasis config                    # Interactive universal config generator

    * oasis config claude             # Auto-configure Claude Desktop

    * oasis config --quick            # Quick universal config with defaults
    """
    try:
        from oasis import mcp_client_configs

        script_dir = Path(mcp_client_configs.__file__).parent
    except ImportError:
        error("Could not find oasis.mcp_client_configs package")
        raise typer.Exit(code=1)
    ...
    if client == "claude":
        ...
        if skills:
            from oasis.skills import AI_TOOLS, install_skills
        ...
        if quick:
            info("Generating OASIS MCP configuration...")
        else:
            info("Starting interactive OASIS MCP configuration...")
