...
This module defines the core Tool protocol that all OASIS tools must implement.
...
@runtime_checkable
class Tool(Protocol):
    """Protocol defining the interface for all OASIS tools.

    Tools must implement this protocol to be registered and used in OASIS.
    ...
    def invoke(self, dataset: DatasetDefinition, params: ToolInput) -> Any:
        ...
        Raises:
            OASISError: For expected errors (query failures, security violations)
