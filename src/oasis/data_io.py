...
def _csv_to_parquet_all(src_root: Path, parquet_root: Path) -> bool:
    ...
    - Tunable via env:
        OASIS_CONVERT_MAX_WORKERS (default: 4)
        OASIS_DUCKDB_MEM         (default: 3GB)
    ...
    def _convert_one(csv_file: Path) -> tuple[Path | None, float, str]:
        ...
        con = duckdb.connect()
        try:
            mem_limit = os.environ.get("OASIS_DUCKDB_MEM", "3GB")
            threads = int(os.environ.get("OASIS_DUCKDB_THREADS", "2"))
    ...
    max_workers = max(1, int(os.environ.get("OASIS_CONVERT_MAX_WORKERS", "4")))
    ...
    try:
        ...
        if "Could not set lock" in str(e):
            logger.error(
                ...
                "Close any running OASIS servers or other DuckDB connections "
...
def ensure_duckdb_for_dataset(
    ...
    if not parquet_root or not parquet_root.exists():
        logger.error(
            f"Parquet directory missing: {parquet_root}. Expected at <project_root>/oasis_data/parquet/{dataset_key}/"
        )
